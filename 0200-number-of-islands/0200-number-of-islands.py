...
class Solution:
    ...
    def check_one(self, i, j, grid):
        # Check if the cell is within bounds and if it is '1'
    ...
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        # Create a grid surrounded by water
        for i in range(m + 2):
        ...
        for i in range(1, m + 1):
            for j in range(1, n + 1):
                if water_surround_grid[i][j] == "1" and not found_lands[i][j]:
                    islands += 1
                    adjacent_lands = self.BFS(water_surround_grid, (i, j))
                    for x, y in adjacent_lands:
